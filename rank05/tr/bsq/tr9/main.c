...
void print_m(t_e *e, t_m *m, t_s *s){
...
int e_b(FILE *file){
    t_e e;
    if(l_e(file, &e) == -1){ printf("Error: load elements error"); return -1;}
    t_m m;
    if(l_m(file, &e, &m) == -1){ printf("Error: Your map is crazy"); return -1;}
    t_s s;
    s.size = 0;
    s.i = 0;
    s.j = 0;
